//﻿<!--
//本日历控件为WalkingPoison委托站长帮手网(http://www.linkhelper.cn)首发，请保留此处信息。
...
//  本日历的year份限制是（1000 - 9999）

//  按ESC键close该控件
//  在year和month的显示地方点击时会分别出year与month的下拉框
...
Ver 2.55
修改日期：2007-6-26
1.year份的下拉框增加了可选项，用于决定year份的滚动是通过点击还是mouseover来触发
...
2.增加了日历中翻yearmonth按钮的双击处理，使得点击的响应更快速
...
1.增加了setYearMonth、setTheme、setYear和setMonth方法，以便外部修改yearmonth的时候调用

2.*重新写了yearmonth的下拉列表，不再使用select控件来选择yearmonth，使得选择yearmonth的操作变得简单
...
修改日期：2002-12-4
1.选中的日期显示为凹下去的样式
2.修改了close层的方法，使得失去焦点的时候能够close日历。

3.修改按键处理，使得Tab切换焦点的时候可以close控件
...
Ver 1.4
修改日期：2002-12-3
1.修正选中year/month份下拉框后按Esc键导致year/month份不显示的问题
2.修正使用下拉框选择month份造成的日期错误（字符串转化为数字的问题）
3.*外观样式的改进，使得控件从丑小鸭变成了美丽的天鹅，从灰姑娘变成了高贵的公主，从……（读者可以自己进行恰当的比喻）

4.再次增大year/month份的点击空间，并对下拉框的位置稍作调整

Ver 1.3
修改日期：2002-11-29
1.*空白部分用灰色显示上个month的最后几天和下个month的前几天

2.*每个日期上面加上鼠标提示
3.修改使得today和当前选择的日期的背景色在灰色日期部分也能正常显示

Ver 1.2
修改日期：2002-11-28
1.*修改year和month的点击都把中文包含在内，增大点击的空间
2.当前选择的日期在列表中显示不同的背景色

3.修正了点击单元格之间的分隔线导致控件close的问题


Ver 1.1
修改日期：2002-11-15
1.修正了方法二按Esc键close以后再次点击不会显示日历的问题
2.点击today直接选中当前的日期并close控件
3.*如果调用控件的输入框含有合法日期，则自动显示输入框的日期部分。

4.修改程序统一使用close的函数closeCalendar()来close日历控件，这样可以通过自定义close函数来完成用户自定义的功能。
...
function wpCalendar(){
	...
	this.ScrollYearOnClick	= false;			//设置year的下拉选择框内的year份是通过点击还是mouseover来滚动
	...
	this.year		= new Date().getFullYear();	//定义year的变量的初始值

	this.month		= new Date().getMonth()+1;	//定义month的变量的初始值
	...
	this.redraw		= function (){				//定义redraw方法，用于重绘整个日历
		...
		strFrame+='document.onmousedown=function(){	/*点击的时候closeyear份和month份的选择列表*/';
		...
		strFrame+='function mOver(obj){strTempClass=obj.className;obj.className="mouseoverYM";bYearListOpening=false;/*为了显示year份列表的时候向上滚动功能的正常使用*/}/*year/month选择列表的mouseover处理*/';
		strFrame+='function mOut(obj){obj.className=strTempClass;}/*year/month选择列表的mouseout处理*/';
		strFrame+='function setMonth(iMonth){parent.wpCalendar.setMonth(iMonth)}/*下拉选择month份的处理*/';
		strFrame+='function setYear(iYear){parent.wpCalendar.setYear(iYear)}/*下拉选择year份的处理*/';
		strFrame+='var bScrolling=false;var bYearListOpening=true;/*这个变量用于控制在刚刚显示year份列表的时候，鼠标处于向上滚动条位置的情况下不要滚动*/';
		strFrame+='var iScroll=null;';
		strFrame+='var bFromYear=false;	/*标志为从year份下拉列表处进行了点击，以便不要closeyear份列表*/';
		...
		strFrame+='			oTd.innerHTML=(iMin-1).toString()+" year";oTd.onmouseover=Function("mOver(this)");oTd.onmouseout=Function("mOut(this)");';
		...
		strFrame+='			oTd.innerHTML=(iMax+1).toString()+" year";oTd.onmouseover=Function("mOver(this)");oTd.onmouseout=Function("mOut(this)");';
		...
		with(this){
			...
			strFrame+='      <tr align=center><td width=16><input title=" 1 month" type=button ';
			...
			strFrame+='onclick="parent.wpCalendar.selectYear(this.innerHTML.substring(0,4))" title="点击这里选择year份" id=CalendarYearHead></td>';
			...
			strFrame+='        title="点击这里选择month份" id=CalendarMonthHead></td>';
			...
			strFrame+='             onfocus="this.blur()" title=" 1 month" style="font-size: 12px; width:16px; height: ' + (cellheight-2) + 'px"></td></tr>';
			...
			strFrame+='<tr align=center valign=bottom><td class=dragbar>Su</td>';
			strFrame+='<td class=dragbar>Mo</td><td class=dragbar>Tu</td>';
			strFrame+='<td class=dragbar>We</td><td class=dragbar>Th</td>';
			strFrame+='<td class=dragbar>Fr</td><td class=dragbar>Sa</td></tr>';
			strFrame+='</table></td></tr>';
			...
			strFrame+='          <tr><td align=left><table border=0 cellspacing=0 cellpadding=0><tr><td><input type=button value="<<" title=" 1 year" onclick="parent.wpCalendar.goPrevYear()" ';
			strFrame+='             ondblclick="parent.wpCalendar.goPrevYear()" onfocus="this.blur()" style="width:20px;font-size: 12px; height: ' + (cellheight) + 'px"><td width="1"></td><td><input title=" 1 month" type=button ';
			...
			strFrame+='             onfocus="this.blur()" title="today" style="font-size: 12px; height: ' + (cellheight) + 'px; cursor:pointer"></td><td ';
			...
			strFrame+='             ondblclick="parent.wpCalendar.goNextMonth()" onfocus="this.blur()" title=" 1 month" style="width:16px;font-size: 12px; height: ' + (cellheight) + 'px"><td width="1"></td><td><input ';
			strFrame+='             type=button value=">>" title=" 1 year" onclick="parent.wpCalendar.goNextYear()" ondblclick="parent.wpCalendar.goNextYear()"';
	...
	this.getYearWidth = function(){				//获取显示year份部分的宽度

		return parseInt(((this.cellwidth*7+2)-32)*0.53);
	};
	this.getMonthWidth = function(){			//获取显示month份部分的宽度
	...
	this.selectMonth = function (strMonth){		//显示month份选择列表
		if (strMonth.match(/\D/)!=null)return;
		var m = (strMonth) ? strMonth : new Date().getMonth() + 1;
		var s = '';
		s += '<table border=0 cellspacing=0 cellpadding=0 class="month">';
		for(var i=1;i<=12;i++){
			s += '<tr><td class=' + (i==parseInt(strMonth)?'selectedYM':'normalYM') + ' onmouseover="mOver(this)" onmouseout="mOut(this)" onmousedown="setMonth('+i+')">' + i + ' Month' + '</td></tr>';
		}
		s += '</table>';
	...
	this.selectYear = function (strYear){		//显示year份选择列表，并且year份列表可以滚动
		...
		with(this.theme){
			...
			if(this.ScrollYearOnClick){
				s += '<tr><td height=5 bgcolor='+bgColorLight+' onmouseover="this.bgColor=\''+bgColorMain+'\'" onmouseout="stopScroll();this.bgColor=\''+bgColorLight+'\'"' +
					'onclick="return false" onmousedown="startScroll(this,true)" onmouseup="stopScroll()" title="向上滚动year份"></td></tr>';
			}
			else{
				s += '<tr><td height=5 bgcolor='+bgColorLight+' onmouseover="startScroll(this,true);this.bgColor=\''+bgColorMain+'\'" onmouseout="stopScroll();this.bgColor=\''+bgColorLight+'\'"' +
					'onclick="return false" title="向上滚动year份"></td></tr>';
			}
			for(var i=n;i<n+12;i++){
				s += '<tr><td class=' + (i==parseInt(strYear)?'selectedYM':'normalYM') + ' onmouseover="mOver(this)" onmouseout="mOut(this)" onmousedown="setYear('+i+')">' + i + ' Year' + '</td></tr>';
			}
			if(this.ScrollYearOnClick){
				s += '<tr><td height=5 bgcolor=' + this.theme.bgColorLight + ' onmouseover="this.bgColor=\''+bgColorMain+'\'" onmouseout="stopScroll();this.bgColor=\''+bgColorLight+'\'"' +
					'onclick="return false" onmousedown="startScroll(this,false)" onmouseup="stopScroll()" title="向下滚动year份"></td></tr>';
			}
			else{
				s += '<tr><td height=5 bgcolor=' + this.theme.bgColorLight + ' onmouseover="startScroll(this,false);this.bgColor=\''+bgColorMain+'\'" onmouseout="stopScroll();this.bgColor=\''+bgColorLight+'\'"' +
					'onclick="return false" title="向下滚动year份"></td></tr>';
			}
			s += '</table>';
		}
	...
	function getDaysOfMonth(year,month){	//得到某year某month的天数
	...
	this.goPrevMonth=function (){		//往前翻month份
		if(this.month>1){this.month--}else{this.year--;this.month=12;}
		this.writeCalendar();
	};
	this.goNextMonth=function (){		//往后翻month份
	...
	this.setMonth=function (i){			//设置日历的month份为month的值，其范围为1~12。

		if(isNaN(i)||i<=0||i>12)return;
		this.month=parseInt(i);
		this.writeCalendar();
	};
	this.setYear=function (i){			//设置日历的year份为year的值，其范围为1000~9999。

		if(isNaN(i)||i<=999||i>9999)return;
		this.year=parseInt(i);
		this.writeCalendar();
	};
	this.setYearMonth=function (year, month){	//设置日历的yearmonth分别为year和month，yearmonth的范围同上。
	...
	this.writeCalendar=function (){	//主要的写程序**********
		...
		var day1 = 1,day2=1,firstday = new Date(yy,mm-1,1).getDay();  //某month第一天的星期几

		var year, month;
		for (i=0;i<firstday;i++){	//上个month的部分
			year = mm==1?yy-1:yy;
			month= mm==1?12:mm-1;
			this.Days[i]=new wpDay(year, month, getDaysOfMonth(year,month)-firstday+i+1);	//上个month的最后几天

		}
		for (i = firstday; day1 < getDaysOfMonth(yy,mm)+1; i++){	//本month的部分

			this.Days[i]=new wpDay(yy, mm, day1);day1++;
		}
		for (i=firstday+getDaysOfMonth(yy,mm);i<39;i++){	//下个month的部分
			year = mm==12?yy+1:yy;
			month= mm==12?1:mm+1;
	...
	//抽象的日期对象，初始化接受yearmonth日的参数，month的范围是1到12
...
function CalendarTheme(){
	...
	this.grayDayColor		= "";	//非本month日期的背景颜色
	this.grayDayFontColor	= "";	//非本month日期的字体颜色
	this.todayColor			= "";	//today的背景颜色

	this.todayFontColor		= "";	//today的字体颜色
...
function showCalendar() //主调函数
{
	...
	function getIndex(){	//根据日期格式设置返回yearmonth日分别所在的位置 //added in Ver 2.1
		var i,j;
...
wpEvents.addListener(document,"click",function(){		//任意点击时close该控件
	with(window.event)
		if (srcElement != wpCalendar.tgtObject && srcElement != wpCalendar.srcButton)
		wpCalendar.close();
});

wpEvents.addListener(document,"keyup",function(){		//按Esc键close，切换焦点close
...
var theme=new Object();
	...
	theme["Classic"].versioninfo		= "";	//主题的版本信息
	...
	theme["Classic"].grayDayColor		= "#E0E0E0";	//非本month日期的背景颜色
	theme["Classic"].grayDayFontColor	= "#808080";	//非本month日期的字体颜色
	theme["Classic"].todayColor			= "#FFD700";	//today的背景颜色

	theme["Classic"].todayFontColor		= "#000000";	//today的字体颜色
	...
	theme["NewAge"].grayDayColor		= "#fff0f5";	//非本month日期的背景颜色
	theme["NewAge"].grayDayFontColor	= "#808080";	//非本month日期的字体颜色
	theme["NewAge"].todayColor			= "#add8e6";	//today的背景颜色

	theme["NewAge"].todayFontColor		= "#000000";	//today的字体颜色
	...
	theme["Nostalgia"].grayDayColor			= "#ffefd5";	//非本month日期的背景颜色
	theme["Nostalgia"].grayDayFontColor		= "#808080";	//非本month日期的字体颜色
	theme["Nostalgia"].todayColor			= "#FFD700";	//today的背景颜色

	theme["Nostalgia"].todayFontColor		= "#000000";	//today的字体颜色
	...
	theme["Icicle"].grayDayColor		= "#e0e9ff";	//非本month日期的背景颜色
	theme["Icicle"].grayDayFontColor	= "#808080";	//非本month日期的字体颜色
	theme["Icicle"].todayColor			= "#FFD700";	//today的背景颜色

	theme["Icicle"].todayFontColor		= "#336699";	//today的字体颜色
	...
	theme["Greenbrier"].grayDayColor		= "#FFF8DC";	//非本month日期的背景颜色
	theme["Greenbrier"].grayDayFontColor	= "#808080";	//非本month日期的字体颜色
	theme["Greenbrier"].todayColor			= "#FFD700";	//today的背景颜色

	theme["Greenbrier"].todayFontColor		= "#000000";	//today的字体颜色
	...
	theme["DiabloII"].grayDayColor			= "#333333";	//非本month日期的背景颜色
	theme["DiabloII"].grayDayFontColor		= "#afafaf";	//非本month日期的字体颜色
	theme["DiabloII"].todayColor			= "#333333";	//today的背景颜色

	theme["DiabloII"].todayFontColor		= "#2e8b57";	//today的字体颜色
